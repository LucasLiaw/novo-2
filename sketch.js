...
function preload(){
  ...
  energyImg = loadImage("assets/kamehameha.png");
...
invisibleGround = createSprite(20,500,10,10000)
...
if(keyWentDown("space")){
  bullet = createSprite(player.x+70,player.y-30,20,10)
  bullet.velocityX = 20
...
if(zombieGroup.isTouching(invisibleGround)){
  for(var i=0;i<zombieGroup.length;i++){     
    ...
    if(zombieGroup[i].isTouching(invisibleGround)){
         zombieGroup[i].destroy()
         life = life - 1;
         } 
...
if(bulletGroup.isTouching(energyGroup)){
bullets = bullets+1
...
function enemy(){
  if(frameCount%50===0){
    ...
    zombie = createSprite(random(800,1300),random(100,500),40,40)

    zombie.addImage(zombieImg)
    zombie.scale = 0.15
    zombie.velocityX = -(3+1*score);
    zombie.debug= false;
